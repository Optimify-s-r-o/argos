...
export const Table = (props: TableProps) => {
  ...
  const [headers, setHeaders] = React.useState<Array<string>>(props.headers);

  React.useEffect(() => {
    let newSort = getDefaultSort();
    let newSortOrder: number[] = [];
    let headerKeysMapping: number[] = [];
...
const TableHeading = styled.th`
  padding: ${(props) =>
    props.tableStyle === TABLE_STYLE_CONDENSED ? '2px 8px' : '8px 16px'};
...
const TableRow = styled.tr`
  ...
  &:hover {
    background-color: ${(props) =>
      getColorWithOpacity(props.theme.colors.accent, 10)};
  }
...
const TableData = styled.td`
  ...
  &:not(:last-child) {
    box-shadow: 1px 0 0 0 ${(props) => props.theme.colors.primary};
  }
