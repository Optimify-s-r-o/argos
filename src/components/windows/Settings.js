            ...
            <div key="content" className="row">
                <div className="column settings-column">
                    <div className="form-card">
                            ...
                            <div className="form-card-row-content">
                                <div className="settings-row">
                                    <div className="pamba-path-column">
                                        <div className="original padding">{truncateMiddle(this.state.pambaPathOriginal, 30, 30, '. . .')}</div>
                                        {this.state.pambaPath !== this.state.pambaPathOriginal ? [
                                            <div className="settings-vertical-arrow">↓</div>,
                                            <div className="padding new">{truncateMiddle(this.state.pambaPath, 30, 30, '. . .')}</div>
                                        ] : ''}
                                    </div>
                                    <div className="pamba-path-column">
                                        <button className="btn btn-text change-setting" onClick={this.openFolderSelection}>{t('settings:common.pambaPathChange')}</button>
                                        {this.state.pambaPath !== this.state.pambaPathOriginal ? <a className="cancel-editing no-margin" onClick={() => this.revert('pambaPath')}>🞨</a> : ''}
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {phases.map(phase => {
                            return <FormRow title={t('settings:capacities.' + phase)}>
                                <div className="settings-row-capacity">
                                    <div className="original capacity">{this.state.settings[phase + 'DefaultCapacity']}</div>
                                    {this.state.settingsEdited.hasOwnProperty(phase + 'DefaultCapacity') ?
                                        [
                                            <span className="settings-horizontal-arrow">→</span>,
                                            <NumericInput
                                                value={this.state.settingsEdited[phase + 'DefaultCapacity']}
                                                onChange={value => {this.numericInputChanged(phase + 'DefaultCapacity', value)}}
                                                min={0}
                                                step={1}
                                                strict={true}
                                                style={NumericInputStyles}
                                            />,
                                            <a className="cancel-editing" onClick={() => this.cancelEdit(phase + 'DefaultCapacity')}>🞨</a>
                                        ] : <button className="btn btn-text change-setting" onClick={() => this.edit(phase + 'DefaultCapacity')}>{t('settings:capacities.edit')}</button>
                                    }
                                </div>
                            </FormRow>
                        })}
