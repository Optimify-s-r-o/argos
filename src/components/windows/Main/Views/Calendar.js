...
class CalendarComponent extends React.Component {
  constructor(props) {
    ...
    this.contextMenuMoveFreeCapacity = this.contextMenuMoveFreeCapacity.bind(
      this
    );
    this.contextMenuChangeTransportInfo = this.contextMenuChangeTransportInfo.bind(
      this
    );
  ...
  componentDidMount() {
    accountGetToken(this.props.settings.url, (res) => {
      // TODO: move to login
      if (res.status === 200) this.props.setToken(res.body);
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.days !== this.props.days) this.fetchJobs();
  }
  ...
  handleMouseEnterRowJob(index) {
    if (index === 0) this.setState({ showJobsShadow: true });
  }

  handleMouseLeaveRowJob(index) {
    if (index === 0) this.setState({ showJobsShadow: false });
  }
  ...
  render() {
    ...
    return (
      <div id='Calendar' className={this.state.isLoadingJobs ? 'loading' : ''}>
        <WeekSelector />
        <RowDays />
        <RowEvents />
        <RowCapacities />
        <div
          ...
          className={
            'view' +
            this.props.calendarView.charAt(0).toUpperCase() +
            this.props.calendarView.slice(1) +
            (this.state.showJobsShadow ? ' show-shadow' : '')
          }
        >
          {Object.keys(this.props.jobs).map((job, index) => {
            return (
              <div
                key={'job-' + job}
                onMouseEnter={(e) => this.handleMouseEnterRowJob(index)}
                onMouseLeave={(e) => this.handleMouseLeaveRowJob(index)}
              >
                <RowJob jobId={job} />
              </div>
            );
          ...
          <ContextMenu id={'PhaseSawMenu'}>
            <MenuItem onClick={this.contextMenuChangeCapacity}>
              {t('calendar:rowJob.phaseContextMenu.changeCapacity')}
            </MenuItem>
            <MenuItem onClick={this.contextMenuMoveFreeCapacity}>
              {t('calendar:rowJob.phaseContextMenu.moveFreeCapacity')}
            </MenuItem>
            <MenuItem divider />
            <MenuItem onClick={this.contextMenuRemovePhase}>
          ...
          <ContextMenu id={'PhaseTransportMenu'}>
            <MenuItem onClick={this.contextMenuChangeTransportInfo}>
          ...
          <ContextMenu id={'EmptyPhaseMenu'}>
            <MenuItem onClick={this.contextMenuCreatePhase}>
...
const Calendar = withTranslation()(
