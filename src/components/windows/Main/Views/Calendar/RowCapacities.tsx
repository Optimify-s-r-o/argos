...
const RowCapacitiesComponent = (props: RowCapacitiesProps) => {
  ...
  const openEditWindow = (date: string, phase: string) => {
    openWindow(
      DayCapacityEditPathWithParams(props.token, date, phase),
      DayCapacityEditSettings
    );
  };
            ...
            >
              {phases.map((phase) => {
                return (
                  <CapacitySection
                    phase={phase}
                    onClick={() => openEditWindow(getDateString(day), phase)}
                  >
                    <CircularProgressbar
